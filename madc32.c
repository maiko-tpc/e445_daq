...
void madc32_irq_vector(unsigned int maddr, short val){
  vwrite16(maddr+MADC32_IRQ_VECTOR, &val);
}

void madc32_read_irq_vector(unsigned int maddr){
  volatile short vector;
  vread16(maddr + MADC32_IRQ_VECTOR, (short *)&vector);
}
...
void madc32_withdraw_irq(unsigned int maddr, short val){
  vwrite16(maddr+MADC32_WITHDRAW_IRQ, &val);
}
